function fetchInfo() {
    ...
    fetch(`/api/servers`)
        ...
        .then(data => {
            server.id = serverId;
            server.name = data.servers.find(server => server.id === serverId).name;
        })
        .catch(error => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const { server, channels, roles } = fetchInfo();
    ...
    try {
        ...
        // set header text
        header.textContent = `Settings for ${server.name}`;
        ...
        fetch('/api/settings/?serverId=' + server.id)
